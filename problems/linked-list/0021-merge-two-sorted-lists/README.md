...
| 回 | 日付 | 結果 | 所要時間 | コード | メモ |
| --- | --- | --- | --- | --- | --- |
| 1 | 2026-07-04 | AC | - | [solution.py](solution.py) | Beats 100%。デバッグ用の print が残っている |
...
ループの末尾に `print()` が 1 行残っていて、要素ごとに空行を出力している。
提出時は動作に影響しないが、消し忘れ。コメントアウトされた print 群は
**値を追いながら組み立てた跡**で、それ自体は良い進め方。
